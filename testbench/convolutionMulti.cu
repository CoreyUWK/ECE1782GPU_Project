...
// could read coalesed by copying to shared memory and then reorder in shared
// memory linearly

/*__global__ void kernel_multi(int in_cols, int in_rows, float *inCh, int
filterSize, bool isSingle, bool isFirst, bool isLast, int totalPaddingHeight,
int totalPaddingWidth, int topPadding, int bottomPadding, int leftPadding, int
rightPadding) { printf("%d %d %d %d %d %d", totalPaddingHeight,
totalPaddingWidth, topPadding, bottomPadding, leftPadding, rightPadding); #ifdef
SHMEM device_CNN_Multi_SHMEM(in_cols, in_rows, inCh, filterSize, isSingle,
isFirst, isLast, totalPaddingHeight, totalPaddingWidth, topPadding,
bottomPadding, leftPadding, rightPadding); #else device_CNN_Multi(in_cols,
in_rows);/*, inCh, filterSize, isSingle, isFirst, isLast, totalPaddingHeight,
totalPaddingWidth, topPadding, bottomPadding, leftPadding, rightPadding);*/
/*#endif
}*/

void setUpCNNFilters(float *host_cov_b, float *host_cov_filter) {
    gpuErrchk(cudaMemcpyToSymbol(device_cov1_b, host_cov_b,
...
void layer1_cov1_multi(int bytes, dim3 grid, dim3 block, float *h_input,
                       float *d_input, float *d_output[COV1_FILTER_OUT_CH]) {
    ...
    for (int ch = 0; ch < COV1_FILTER_OUT_CH; ++ch) {
        ...
        gpuErrchk(cudaMemcpyToSymbol(device_output[0], &d_output[ch],
                                     sizeof(float *), ch * sizeof(float *),
                                     cudaMemcpyHostToDevice));
    }
    ...
    device_CNN_Multi_SHMEM<<<grid, block, shmemSize>>>(
        INPUT_WIDTH, INPUT_HEIGHT, d_input, COV1_FILTER_N, true, false, false,
        totalPaddingHeight, totalPaddingWidth, topPadding, bottomPadding,
        leftPadding, rightPadding);
#else
    device_CNN_Multi_v1<<<grid, block>>>(
        INPUT_WIDTH, INPUT_HEIGHT, d_input, COV1_FILTER_N, true, false, false,
        totalPaddingHeight, totalPaddingWidth, topPadding, bottomPadding,
        leftPadding, rightPadding);
#endif
...
void layer1_maxPool_multi(int in_cols, int in_rows, float **d_input,
    ...
    dim3 block((out_cols < 32) ? out_cols : 32,
               (out_rows < 32) ? out_rows : 32);
    ...
    for (int ch = 0; ch < COV1_FILTER_OUT_CH; ++ch) {
        ...
        max_pool_2d<<<grid, block /*, 0, streams[ch]*/>>>(
            d_input[ch], in_rows, in_cols, d_output[ch], out_rows, out_cols);
    }
...
void layer2_cov_multi(int filterSize, int in_cols, int in_rows, float **d_input,
                      float **d_output) {
    ...
    getConvPadding(filterSize, totalPaddingHeight, totalPaddingWidth,
                   topPadding, leftPadding, bottomPadding, rightPadding);
    // printf("%d %d %d %d\n", totalPaddingHeight, totalPaddingWidth,
    // topPadding, leftPadding);

#ifdef SHMEM
    const int shmemSize = (in_rows + totalPaddingHeight) *
                          (in_cols + totalPaddingWidth) * sizeof(float);
#endif
    for (int ch = 0; ch < COV2_FILTER_OUT_CH; ++ch) {
        // Setup all filters and b
        setUpCNNFilters(
            host_cov1_b,
            &host_cov1_filter[0][0][0][0]); // TODO UPdate for each iteration

#ifdef SHMEM
        device_CNN_Multi_SHMEM<<<grid, block, shmemSize>>>(
            in_cols, in_rows, d_input[ch], filterSize, false,
            (ch == 0) ? true : false,
            (ch == (COV2_FILTER_OUT_CH - 1)) ? true : false, totalPaddingHeight,
...
int main(int argc, char *argv[]) {
    ...
    float *h_linear_out = (float *)malloc(
        OUTPUT_SIZE2 * sizeof(float)); // host output of linear layers
    // pinning host memory
    gpuErrchk(
        cudaHostRegister(h_W1, INPUT_SIZE1 * OUTPUT_SIZE1 * sizeof(float), 0));
    gpuErrchk(cudaHostRegister(h_b1, OUTPUT_SIZE1 * sizeof(float), 0));
    gpuErrchk(
        cudaHostRegister(h_W2, INPUT_SIZE2 * OUTPUT_SIZE2 * sizeof(float), 0));
    ...
    layer1_maxPool_multi(INPUT_WIDTH, INPUT_HEIGHT, d_in, out_col, out_row,
                         d_out);
    ...
    // Perform second max pooling on the output from the second convolution
    // layer
    std::copy(d_out, d_out + COV2_FILTER_OUT_CH, d_in);
    layer1_maxPool_multi(out_col, out_row, d_in, out_col, out_row, d_out);
    ...
    float *d_flattened;
    gpuErrchk(cudaMalloc((void **)&d_flattened, COV3_FILTER_OUT_CH * out_col *
                                                    out_row * sizeof(float)));
    for (int ch = 0; ch < COV3_FILTER_OUT_CH; ch++) {
    ...
    float *d_linear_out2;
    gpuErrchk(
        cudaMalloc((void **)&d_W1, INPUT_SIZE1 * OUTPUT_SIZE1 * sizeof(float)));
    gpuErrchk(cudaMalloc((void **)&d_b1, OUTPUT_SIZE1 * sizeof(float)));
    gpuErrchk(
        cudaMalloc((void **)&d_W2, INPUT_SIZE2 * OUTPUT_SIZE2 * sizeof(float)));
    gpuErrchk(cudaMalloc((void **)&d_b2, OUTPUT_SIZE2 * sizeof(float)));
    gpuErrchk(
        cudaMalloc((void **)&d_linear_out1, OUTPUT_SIZE1 * sizeof(float)));
    gpuErrchk(
        cudaMalloc((void **)&d_linear_out2, OUTPUT_SIZE2 * sizeof(float)));

    // transfer weights and biases to device
    gpuErrchk(cudaMemcpy(d_W1, h_W1, INPUT_SIZE1 * OUTPUT_SIZE1 * sizeof(float),
                         cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(d_b1, h_b1, OUTPUT_SIZE1 * sizeof(float),
                         cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(d_W2, h_W2, INPUT_SIZE2 * OUTPUT_SIZE2 * sizeof(float),
                         cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(d_b2, h_b2, OUTPUT_SIZE2 * sizeof(float),
                         cudaMemcpyHostToDevice));

    // linear layer 1: input: d_flattened (1x22400) output: (1x256)
    linear<<<64, 1024>>>(d_linear_out1, d_flattened, d_W1, d_b1, INPUT_SIZE1,
                         OUTPUT_SIZE1, false);
    // linear layer 2: input: d_linear_out1 (1x256) output: (1x3)
    linear<<<64, 1024>>>(d_linear_out2, d_linear_out1, d_W2, d_b2, INPUT_SIZE2,
                         OUTPUT_SIZE2, true);
    gpuErrchk(cudaDeviceSynchronize());

    // copy data back
    gpuErrchk(cudaMemcpy(h_linear_out, d_linear_out2,
                         OUTPUT_SIZE2 * sizeof(float), cudaMemcpyDeviceToHost));
    ...
    // #ifdef PRINTDATA
    //     for (int ch=0; ch < COV1_FILTER_OUT_CH; ++ch) {
    //         gpuErrchk(cudaMemcpy(h_output[ch], d_out[ch],
    //         out_col*out_row*sizeof(float), cudaMemcpyDeviceToHost));
    //     //}
    //     //for (int ch=0; ch < COV1_FILTER_OUT_CH; ++ch) {
    ...
    gpuErrchk(cudaMemcpy(h_flattened, d_flattened, flattenedBytes,
    ...
    for (int i = 0; i < COV3_FILTER_OUT_CH; i++) {
        ...
        for (int j = 0; j < out_row; j++) {
            for (int k = 0; k < out_col; k++) {
                printf("%f ",
                       h_flattened[k + j * out_col + i * out_row * out_col]);
            }
            printf("\n");
        }
    }
