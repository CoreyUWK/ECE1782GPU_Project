...
__global__ void device_CNN(int in_cols, int in_rows, float *inCh, float *outCh,
                           float b, float *filter, int filterSize,
    ...
    for (int i = 0, offset = cnnOffset, filterOffset = 0, topChecki = topCheck;
         i < filterSize;
         ++i, offset += in_cols, filterOffset += filterSize, ++topChecki) {
        if (topChecki < 0)
            continue;
        else if (topChecki >= in_rows)
            break;
        for (int j = 0; j < filterSize; ++j) {
            int leftCheckj = leftCheck + j;
            if (leftCheckj < 0)
                continue;
            if (leftCheckj >= in_cols)
                break;
...
void layer_cov(int in_cols, int in_rows, cudaStream_t *streams,
               cudaEvent_t &event, float *h_input, float *d_input,
               float **d_output) {
    ...
    for (int i = 0; i < COV1_FILTER_OUT_CH; ++i) {
        ...
        if (i > 0 && i < 64) // input cpy and first filter run on same stream so
                             // skip on first stream
        ...
        device_CNN<<<grid, block, 0, streams[i]>>>(
            in_cols, in_rows, d_input, d_output[i], host_cov1_b[i],
            filterAddr + i * COV1_FILTER_N * COV1_FILTER_N, COV1_FILTER_N, true,
            false, false, totalPaddingHeight, totalPaddingWidth, topPadding,
            leftPadding);
    }
...
void layer_maxPool(int in_cols, int in_rows, float **d_input, int &out_cols,
                   int &out_rows, float **d_output, cudaStream_t *streams) {
...
void layer_covMulti(int in_cols, int in_rows, cudaStream_t *streams,
                    cudaEvent_t *events, float **d_input, float **d_output,
                    int filterSize) {
    ...
    for (int inCh = 0; inCh < COV2_FILTER_IN_CH; ++inCh) {
        ...
        for (int outCh = 0; outCh < COV2_FILTER_OUT_CH; ++outCh) {
            // Every stream needs to wait for input
            gpuErrchk(cudaStreamWaitEvent(streams[COV2_FILTER_IN_CH + outCh],
                                          events[inCh], 0));

            // Setup all filters and b
            setupFilterCh(outCh, &host_cov1_b[outCh],
                          &host_cov1_filter[0][outCh][0][0],
                          streams[COV2_FILTER_IN_CH + outCh]);

            // Get output memory
            if (inCh == 0) {
                gpuErrchk(cudaMalloc((void **)&d_output[outCh], bytes));
            }
            device_CNN<<<grid, block, 0, streams[COV2_FILTER_IN_CH + outCh]>>>(
                in_cols, in_rows, d_input[inCh], d_output[outCh],
                host_cov1_b[outCh],
                filterAddr + outCh * COV1_FILTER_N * COV1_FILTER_N, filterSize,
...
int main(int argc, char *argv[]) {
    ...
    layer_cov(INPUT_WIDTH, INPUT_HEIGHT, streams, events[0], h_input, d_input,
              d_out);
    ...
    layer_maxPool(INPUT_WIDTH, INPUT_HEIGHT, d_in, out_col, out_row, d_out,
                  streams);
    ...
    layer_covMulti(out_col, out_row, streams, events, d_in, d_out,
                   COV2_FILTER_N);
    ...
    // layer
    std::copy(d_out, d_out + COV2_FILTER_OUT_CH, d_in);
    layer_maxPool(out_col, out_row, d_in, out_col, out_row, d_out,
                  &streams[64]);
    ...
    layer_covMulti(out_col, out_row, streams, &events[64], d_in, d_out,
                   COV3_FILTER_N);
